...
def _original_lookup_block() -> str:
    """The exact block we are going to replace."""
...
def _patched_lookup_block() -> str:
    """Replacement block that normalizes the path before lookup."""
    return """    for func_group in func_sign_groups:
        {marker}
        enbale_flag = enable_ascendc_time_stamp.get(func_group.filepath)
...
def apply_patch(backup_path: Path | None = None) -> Path:
    ...
    old_block = _original_lookup_block()
    ...
    patched_text = original_text.replace(old_block, _patched_lookup_block(), 1)
